...
| M1 | Valid dual-source risk; breaking proposal rejected | `_transport` and `_profile` are authoritative for owner behavior. The retained `Config` object is now explicitly a legacy compatibility/observed view, preserving source/API compatibility (including `getConfig()` reference semantics) and raw-register diagnostics. Raw Configuration writes do not destroy the desired profile needed by reconcile/recovery. |
...
| M3 | Valid | A central failure terminalizer consistently maps `DEADLINE_EXPIRED` to `TIMED_OUT`, distinguishes ambiguous invoked writes, and reports `PARTIAL` only after a confirmed earlier write. Interior zero-share tests prove zero callbacks both before I/O (`HardwareEffect::NONE`) and after an exact trigger write (`HardwareEffect::PARTIAL`). Because a trigger rewrites the already-verified desired Configuration value, later sample failure does not falsely make register certainty `DIRTY`. |
...
| M5 | Valid fabricated-observation defect; proposed clear rejected | Software reset no longer fabricates a zero Mask/Enable reading. A confirmed reset clears the known POR writable-bit cache; an ambiguous reset preserves the last real observation while certainty becomes `UNKNOWN`. Host-retained alert history remains untouched until the application takes it. Clearing the entire snapshot, as proposed, would silently acknowledge previously observed events. |
...
| L3 | Valid | Legacy conversion accepts missing/invalid shunt values on disabled channels and normalizes them to zero, including zero, NaN, and finite out-of-range values; enabled channels still require representable finite positive calibration. |
...
| L8 | Valid | Corrected absolute capture uptime, callback-count/readiness documentation, and `nowMs` health usage. The timing checker now lexes comments, quoted literals, and C++ raw strings and runs an adversarial self-test, avoiding substitution-order blind spots. The previously reported ignored tarball and stale generated Doxygen directory were absent from the synced worktree; Doxygen was then freshly regenerated. Snapshot cursor fixes from the audit pass are exercised alongside the expanded native suite. |
...
On 2026-09-01, three parallel read-only reviewers independently re-read the
complete original audit from the parent revision, inspected the actual
implementation and `HEAD^..HEAD` diff, and divided coverage across critical/high
findings, medium state-machine semantics, and low/documentation/tooling scope.
Their findings were then reproduced against the code before correction.

The follow-up audit found and fixed these residual edges:

- A definitely non-reaching typed Configuration write no longer discards an
  active or completed legacy conversion; confirmed/ambiguous writes and failed
  verification still clear stale provenance conservatively.
- Raw Configuration compatibility writes now also discard stale legacy
  conversion timing after confirmed or ambiguous writes. Definite address
  NACK/not-found failures preserve the still-valid conversion evidence.
- Every accepted owner job clears completed legacy-ready provenance, while
  active legacy conversions remain rejected only for sample jobs. Successful
  rebind, lifecycle recovery, and completed-ready handoff are now explicit tests.
- The raw Configuration compatibility view now survives host-only shunt updates;
  `getSettings()`, `tickStatus()`, alert CVRF consumption, and no-argument
  `startConversion()` all use or restore the correct observed configuration.
- Ambiguous reset preserves the last observed Mask/Enable writable cache, and a
  successful typed write no longer overwrites the diagnostic that explains a
  pre-existing dirty/unknown register family.
- Callback documentation now distinguishes local validation from physical bus
  access, and owner transfer counters are documented as callback invocations.
  Arduino `endTransmission()` result 1 remains `INVALID_PARAM`: it is the local
  data-too-long result, not evidence of a physical bus failure.
- The 50 ms CVRF fault-recheck contract, bus-only shunt-sum rejection, disabled
  out-of-range shunts, successful rebind, fixed-timeout adapter classification,
