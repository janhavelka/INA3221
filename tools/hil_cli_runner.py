...
def classify_output(
    ...
    for level_label, fault_label in (
        ("TCF", "TC_FAULT"), ("TimingControl", "TimingControlFault")
    ):
        levels = re.findall(rf"\b{level_label}=(\S+)", text)
        faults = re.findall(rf"\b{fault_label}=(\S+)", text)
        if len(levels) != len(faults) or any(
            pair not in (("0", "1"), ("1", "0"))
            for pair in zip(levels, faults)
        ):
            return "FAIL", f"inconsistent {level_label}/{fault_label} levels"
...
def run_parser_self_test() -> int:
    cases = [
        ...
        (
            "accept inactive timing-control fault",
            "TCF=1  TC_FAULT=0  CVRF=0\nTimingControl=1 TimingControlFault=0\n",
            ("TCF=", "TC_FAULT=", "TimingControl=", "TimingControlFault="),
            DEFAULT_FAILURE_TOKENS,
            False,
            "PASS",
        ...
        (
            "accept latched timing-control fault",
            "TCF=0  TC_FAULT=1  CVRF=0\nTimingControl=0 TimingControlFault=1\n",
            ("TCF=", "TC_FAULT=", "TimingControl=", "TimingControlFault="),
            DEFAULT_FAILURE_TOKENS,
            False,
            "PASS",
        ...
        (
            "reject incorrect timing-control fault polarity",
            "TCF=1  TC_FAULT=1  CVRF=0\n",
            ("TCF=", "TC_FAULT="),
            DEFAULT_FAILURE_TOKENS,
            False,
            "FAIL",
        ...
        (
            "reject missing timing-control fault level",
            "TimingControl=1\n",
            ("TimingControl=",),
            DEFAULT_FAILURE_TOKENS,
            False,
        ...
        (
            "reject malformed timing-control fault level",
            "TimingControl=1 TimingControlFault=unknown\n",
            ("TimingControl=", "TimingControlFault="),
            DEFAULT_FAILURE_TOKENS,
            False,
            "FAIL",
        ),
        (
            "reject unverified setter transfer count",
            "XFER_ASSERT PASS read=0 write=1 total=1\n",
            ("XFER_ASSERT PASS read=1 write=1 total=2\n",),
            DEFAULT_FAILURE_TOKENS,
            False,
            "FAIL",
        ),
...
def default_steps(stress_count: int, stress_mix_count: int) -> list[Step]:
    ...
    return [
        ...
        Step("BASE-007A", "Typed-setter transfer counter reset", "xfer_reset",
             ("XFER_RESET read=0 write=0 total=0",)),
        Step("BASE-007B", "Verified averaging setter", "avg 0", ("Status: OK",)),
        Step("BASE-007C", "Typed setter writes and verifies", "xfer_assert 1 1 2",
             ("XFER_ASSERT PASS read=1 write=1 total=2\n",)),
        Step("BASE-007D", "Typed setter retains measurement certainty", "settings",
             ("Hardware config dirty: NO\n",)),
        Step("BASE-007E", "Read immediately after typed setter", "shunt 1",
             ("CH1 shunt",), notes="No recover between setter and measurement."),
        ...
        Step("OWNER-026", "Reconcile job start", "job reconcile",
             ("Status: IN_PROGRESS",)),
        Step("OWNER-026A", "Wire-safe maximum owner budget", "job step 255",
             ("Wire-safe transfer budget:", "Status: OK")),
        ...
        Step("DATA-003T", "Triggered blocking-read profile", "mode sbtrig",
             ("Status: IN_PROGRESS",), pacing_s=0.05),
        Step("DATA-003T1", "Blocking read waits for triggered conversion", "read",
             ("CH1:", "CH2:", "CH3:")),
        Step("DATA-003T2", "Longer triggered averaging", "avg 1", ("Status: OK",)),
        Step("DATA-003T3", "Blocking read waits for averaged conversion", "read",
             ("CH1:", "CH2:", "CH3:")),
        Step("DATA-003T4", "Restore default averaging", "avg 0", ("Status: OK",)),
        ...
        Step("DATA-010A", "Unselected sum transfer counter reset", "xfer_reset",
             ("XFER_RESET read=0 write=0 total=0",)),
        Step("DATA-010B", "Unselected sum is rejected", "sumraw",
             ("Status: INVALID_CONFIG", "No summation channel selected")),
        Step("DATA-010C", "Unselected sum rejection is bus-silent", "xfer_assert 0 0 0",
             ("XFER_ASSERT PASS read=0 write=0 total=0\n",)),
        Step("DATA-010D", "Select a measured summation channel", "sumch 1 1",
             ("Status: OK",)),
        Step("DATA-010E", "Mask setter pre-reads, writes and verifies", "xfer_assert 2 1 3",
             ("XFER_ASSERT PASS read=2 write=1 total=3\n",)),
        Step("DATA-011", "Shunt sum raw", "sumraw", ("Shunt sum raw",)),
        Step("DATA-012", "Shunt sum float", "sum", ("Shunt sum",)),
        Step("DATA-012A", "Restore summation selection", "sumch 1 0", ("Status: OK",)),
        ...
        Step("MODE-016", "Slow cycle for active legacy cancellation", "avg 7",
             ("Status: OK",)),
        Step("MODE-017", "Start cancellable legacy conversion", "start sbtrig",
             ("Status: IN_PROGRESS",), pacing_s=0.0,
             notes="AVG_1024 keeps the conversion active during the following commands."),
        Step("MODE-018", "Legacy cancellation transfer counter reset", "xfer_reset",
             ("XFER_RESET read=0 write=0 total=0",), pacing_s=0.0),
        Step("MODE-019", "Cancel active legacy conversion", "cancel",
             ("Status: OK",), pacing_s=0.0),
        Step("MODE-020", "Legacy cancellation is bus-silent", "xfer_assert 0 0 0",
             ("XFER_ASSERT PASS read=0 write=0 total=0\n",)),
        Step("MODE-021", "Second legacy cancellation has no active conversion", "cancel",
             ("Status: NO_ACTIVE_JOB",)),
        Step("MODE-022", "Continuous restore after cancellation", "mode sbc",
             ("Status: OK",)),
        Step("MODE-023", "Averaging restore after cancellation", "avg 0",
             ("Status: OK",)),
        *[
            step
        ...
        *[
        ...
        Step("ALERT-001", "Alert flags", "alerts",
             ("Alert Flags", "TimingControl=", "TimingControlFault=")),
        Step("ALERT-002", "Mask/Enable decode", "mask",
             ("Mask/Enable Register", "clears latched alert", "TCF=",
              "TC_FAULT=", "TC_FAULT is the inverted TCF level")),
